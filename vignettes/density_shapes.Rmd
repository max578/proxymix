...
vignette: >
  %\VignetteIndexEntry{The wedge: KLD-EM on three density shapes}
...
This is the wedge demonstration. The KLD-EM regime fits a Gaussian-mixture proxy to a target whose density we can *evaluate* but cannot easily *sample from*. Existing CRAN packages for Gaussian mixtures — `mclust`, `mixtools`, `flexmix` — all assume i.i.d. samples are available.
